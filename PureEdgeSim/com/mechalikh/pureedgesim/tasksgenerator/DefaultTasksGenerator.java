...
public class DefaultTasksGenerator extends TasksGenerator {
	...
	public List<Task> generate() {
		...
		SimLog.println("device size : " + devicesList.size());
		int dev = 0;
	...
	//	SimLog.println("device size after remove: " + devicesList.size());
		...
		for (int j = 0; j < devicesList.size(); j++){
			...
			//SimLog.println("j%(SimulationParameters.APPLICATIONS_LIST.size()-1)" + j%(SimulationParameters.APPLICATIONS_LIST.size()));
...
int count = 0;
	private void generateTasksForDevice(ComputingNode dev, int app) {
		...
		SimLog.println("count: " + count++);
		for (int st = 0; st < 1; st++) { // for each minute
